...
theorem quartic_factorization (ρ : ℝ) :
  ...
  constructor
  · simp only [quartic, quotientCubic, conjugate_leadingScalar,
      conjugate_middleScalar, leadingScalar, middleScalar, centralScalar,
      map_add, map_sub, map_mul, map_pow, map_ofNat]
    ring
  · constructor
    all_goals
      simp only [quotientCubic, eval_sub, eval_add, eval_mul, eval_pow, eval_C,
        eval_X, conjugate_leadingScalar, conjugate_middleScalar,
        leadingScalar, middleScalar]
      ring
...
theorem cayley_identity (ρ : ℝ) (x : ℂ) (hx : 1 - Complex.I * x ≠ 0) :
  ...
  simp only [quotientCubic, eval_sub, eval_add, eval_mul, eval_pow, eval_C,
    eval_X, conjugate_leadingScalar, conjugate_middleScalar,
    leadingScalar, middleScalar, cayley]
  field_simp [hx] <;>
...
theorem exceptional_parameter (ρ : ℝ) (hρ : 0 < ρ) (hρ2 : ρ ^ 2 = 10) :
  ...
  have he : (quotientCubic ρ).derivative.eval (-1) =
    ...
    simp [quotientCubic, conjugate_leadingScalar, conjugate_middleScalar,
      leadingScalar, middleScalar, hc] <;> ring
  refine ⟨?_, ?_, ?_, he, ?_⟩
  · simp [realCubic, Cubic.toPoly, hρ2]
  · nlinarith [hρ2]
