...
lemma source_block_equation {n : ℕ} (ρ : ℝ) (x : BlockVector n) (j : Fin n) :
  ...
  ext c
  fin_cases c
  · change Matrix.mulVec (scaledToeplitz ρ n) x (j, 0) = _
    rw [source_row_zero]
  ...
  · change Matrix.mulVec (scaledToeplitz ρ n) x (j, 1) = _
    rw [source_row_one]
    simp [leadingBlock, transferRhs, sourceState, Matrix.mulVec, dotProduct,
      Fin.sum_univ_succ, Matrix.cons_val_two, Matrix.cons_val_three,
      Matrix.vecHead, Matrix.vecTail] <;> ring
...
lemma source_step_iff {n : ℕ} (ρ : ℝ) (hρ : 0 < ρ)
  ...
  · intro h
    ext i
    fin_cases i
    · simpa [v, b, sourceState, Nat.cast_add] using congrFun h 0
    · simpa [v, b, sourceState, Nat.cast_add] using congrFun h 1
