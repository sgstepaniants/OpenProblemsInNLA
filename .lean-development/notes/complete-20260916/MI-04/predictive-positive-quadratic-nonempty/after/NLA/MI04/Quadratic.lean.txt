...
lemma realQuadratic_continuous (M : CMatrix ι) : Continuous (realQuadratic M) := by
  have hfun : realQuadratic M =
      (Matrix.toEuclideanCLM (n := ι) (𝕜 := ℂ) M).reApplyInnerSelf :=
    funext (realQuadratic_eq_reApply M)
  rw [hfun]
  exact (Matrix.toEuclideanCLM (n := ι) (𝕜 := ℂ) M).reApplyInnerSelf_continuous
...
lemma realQuadratic_smul_matrix (c : ℝ) (M : CMatrix ι) (v : CVector ι) :
  ...
  simp only [realQuadratic, map_smul, ContinuousLinearMap.smul_apply,
...
lemma positive_quadratic_iff [Nonempty ι] (M : CMatrix ι) :
