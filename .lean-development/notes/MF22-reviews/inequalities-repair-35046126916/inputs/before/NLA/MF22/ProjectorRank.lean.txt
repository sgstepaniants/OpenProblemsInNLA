...
lemma transfer_eigenspace_finrank_le_one_of_charpoly (ρ : ℝ)
    ...
    (i : Fin 4) :
    Module.finrank ℂ ((transferMatrix ρ).toLin'.eigenspace (roots i)) ≤ 1 := by
...
lemma entry_minor_zero_of_simple_eigenspace {n : ℕ} (A P : Square n) (z : ℂ)
    (hAP : A * P = z • P)
    (hdim : Module.finrank ℂ (A.toLin'.eigenspace z) ≤ 1)
    (r k c s : Fin n) :
    P r c * P k s = P k c * P r s := by
  let E := A.toLin'.eigenspace z
  have hcol (j : Fin n) : (fun row => P row j) ∈ E := by
    apply Module.End.mem_eigenspace_iff.mpr
    change A *ᵥ (fun row => P row j) = z • (fun row => P row j)
    funext row
